...
async function home(request, env) {
  ...
  return page("Home", `
    <section class="hero">
      ...
      <p>Auto-updating news from NewsData.io.</p>
      <p><a class="button" href="/admin">Admin</a> <a class="button secondary" href="/subscribe">Subscribe</a></p>
    </section>
    <nav class="chips">${cats}</nav>
    <main class="grid">${cards || `<div class="panel"><h2>No articles yet</h2><p>Log into admin and run sync.</p></div>`}</main>
  `, env);
}
...
async function admin(request, env) {
  ...
  const rows = (articles.results || []).map(a => `
    <tr>
      <td>${esc(a.title)}</td>
...
async function deleteArticle(request, env) {
  ...
  const form = await request.formData();
  await env.DB.prepare(`DELETE FROM articles WHERE id = ?`).bind(String(form.get("id") || "")).run();
...
<head>
  ...
  <style>
    ...
    nav{display:flex;gap:14px;color:#a1a1aa}
    .wrap{max-width:1100px;margin:0 auto;padding:28px 18px}
    .hero,.panel,.card{background:#111114;border:1px solid #27272a;border-radius:18px;padding:20px;margin-bottom:16px}
